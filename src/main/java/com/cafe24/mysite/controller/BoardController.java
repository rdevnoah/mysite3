...
public class BoardController {
	...
	public String delete(
		...
		@RequestParam( value="p", required=true, defaultValue="1") Integer page,
		@RequestParam( value="kwd", required=true, defaultValue="") String keyword ) {
		boardService.deleteMessage( boardNo, securityUser.getNo() );
		return "redirect:/board?p=" + page + "&kwd=" + WebUtil.encodeURL( keyword, "UTF-8" );
	}
	
	@RequestMapping( value="/modify/{no}" )	
	public String modify( @AuthUser SecurityUser securityUser, @PathVariable( "no" ) Long no, Model model) {
		BoardVo boardVo = boardService.getMessage(no, securityUser.getNo() );
		model.addAttribute( "boardVo", boardVo );
		return "board/modify";
	}
